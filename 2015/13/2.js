...
Object.keys(initialData.neighbors).forEach((personKey) => {
    initialData.neighbors[personKey]['ITS_ME'] = 0;
});

initialData.neighbors['ITS_ME'] = [...initialData.peoples].reduce((acc, person) => {
    acc[person] = 0;
    return acc;
}, {});

initialData.peoples.add('ITS_ME');

console.log('MY_REG initialData: ', initialData);
...
const result = counts.reduce((acc, count) => {
    if (count > acc) {
        return count;
    }
    return acc;
